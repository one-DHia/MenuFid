...
export default function DriverMobilePortalPage() {
  ...
  async function handleLogin(e: React.FormEvent) {
    ...
    try {
      ...
      if (json.success && json.driver) {
        ...
        showToast(`${t('hello', 'Bonjour')} ${json.driver.name} !`, 'success');
        fetchOrders();
      } else {
        setLoginError(json.error || t('driver_invalid_creds', 'Identifiant ou mot de passe incorrect.'));
      }
    } catch {
      setLoginError(t('driver_network_error', 'Erreur de connexion. Vérifiez votre réseau.'));
    } finally {
  ...
  async function fetchOrders() {
    ...
    try {
      ...
      showToast(t('error_loading_orders', 'Erreur lors du chargement des commandes.'), 'error');
    } finally {
      setLoading(false);
    }
  }
  ...
  function handleLogout() {
    ...
    setDriver(null);
    setOrders([]);
    showToast(t('driver_logout_success', 'Déconnexion réussie.'), 'info');
  }
  ...
  async function handleCompleteDelivery(order: DeliveryOrder) {
    ...
    const confirmMessage = isCash
      ? `${t('driver_confirm_cash_prefix', 'Confirmez-vous avoir encaissé')} ${order.total_amount} ${order.currency} ${t('driver_confirm_cash_suffix', 'en espèces ? Cela créditera 1 point fidélité au client.')}`
      : t('driver_confirm_delivered', 'Confirmez-vous que cette commande a bien été remise au client ?');
    ...
    try {
      ...
      if (json.success) {
        ...
        showToast(json.error || t('driver_action_error', 'Erreur lors de la validation.'), 'error');
      }
    } catch {
      showToast(t('driver_network_action_error', 'Erreur réseau.'), 'error');
    } finally {
  ...
  if (!driver) {
    return (
      <div className="min-h-screen bg-[#FAFAFA] text-black flex flex-col justify-center px-4 py-8 max-w-sm mx-auto select-none" dir={dir}>
        {/* Sélecteur de langue */}
        ...
        <div className="text-center mb-8 space-y-2">
          ...
          <h1 className="text-2xl font-black text-black tracking-tight mt-3">
            {t('driver_portal_title', 'Portail Livreur')}
          </h1>
          <p className="text-xs text-neutral-600 font-bold">
            {t('driver_portal_subtitle', 'MenuFid • Vos livraisons en direct')}
          </p>
        </div>
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            <div>
              <label className="block text-[11px] font-black uppercase text-neutral-700 mb-1">
                {t('driver_username_label', 'Identifiant Livreur')}
              </label>
              <div className="relative">
                <input
                  ...
                  placeholder={t('driver_username_ph', 'Ex: livreur-karim')}
                  className="w-full px-4 py-3 rounded-2xl border-2 border-black text-xs font-bold focus:outline-none focus:ring-2 focus:ring-[#FFB800]"
                />
              </div>
            </div>
            ...
            <div>
              <label className="block text-[11px] font-black uppercase text-neutral-700 mb-1">
                {t('driver_password_label', 'Mot de passe')}
              </label>
              <div className="relative">
                <input
                  type="password"
            ...
            >
              {loginLoading
                ? t('driver_logging_in', 'Connexion en cours...')
                : t('driver_login_btn', 'Accéder à mes livraisons')}
            </button>
          </form>

          <p className="text-[10px] text-neutral-500 text-center font-bold mt-4">
            {t('driver_creds_info', 'Vos identifiants vous sont fournis par le gérant de votre restaurant.')}
          </p>
        </div>
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-[#FAFAFA] text-black pb-12 max-w-md mx-auto flex flex-col font-sans select-none" dir={dir}>
      ...
      <header className="sticky top-0 z-30 bg-white border-b-3 border-black px-4 py-3 flex items-center justify-between shadow-xs">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            ...
            <p className="text-[10px] font-bold text-neutral-500 truncate max-w-[130px]">
        ...
        <div className="flex items-center gap-1.5 sm:gap-2">
          ...
          <button
            ...
            title={t('driver_refresh', 'Rafraîchir')}
          >
          ...
          <button
            ...
            title={t('driver_logout', 'Déconnexion')}
          >
      ...
      <main className="flex-1 p-4 space-y-4">
        ...
        {loading && orders.length === 0 ? (
          <div className="text-center py-16 space-y-3">
            ...
            <p className="text-xs font-bold text-neutral-500">{t('driver_loading_orders', 'Chargement de vos livraisons...')}</p>
          </div>
        ) : orders.length === 0 ? (
          <div className="p-8 bg-white border-3 border-black rounded-3xl text-center space-y-3 shadow-[4px_4px_0px_0px_#000] mt-4">
            ...
            <h3 className="font-black text-sm text-black">{t('driver_all_done_title', 'Toutes les livraisons sont terminées !')}</h3>
            <p className="text-xs text-neutral-500 font-medium">
              {t('driver_all_done_desc', 'Dès qu’une nouvelle commande est prête en cuisine, elle apparaîtra automatiquement ici.')}
            </p>
          </div>
        ) : (
          orders.map((order) => {
                ...
                <div className="flex items-start justify-between border-b-2 border-neutral-100 pb-2.5">
                  <div>
                    ...
                    <p className="text-[11px] font-bold text-neutral-500">
                      {t('driver_client_label', 'Client :')} <strong className="text-black">{order.customer_name}</strong>
                    </p>
                  </div>
                  ...
                  <div className="flex items-start gap-2">
                    ...
                    <div className="flex-1">
                      <p className="text-xs font-bold text-black leading-snug">
                        {order.customer_address || t('driver_no_address', 'Adresse non renseignée')}
                      </p>
                ...
                <div className="pt-2">
                  {isCash ? (
                    ...
                    >
                      ...
                      <span>
                        {isPendingAction
                          ? t('driver_saving', 'Enregistrement...')
                          : `${t('driver_collect_and_deliver', 'Encaisser')} ${order.total_amount} ${order.currency} ${t('driver_and_deliver_point', '& Livrer (+1 Point)')}`}
                      </span>
                    </button>
                  ) : (
                    ...
                    >
                      ...
                      <span>
                        {isPendingAction
                          ? t('driver_validating', 'Validation...')
                          : t('driver_validate_paid', 'Valider la Livraison (Déjà Payé)')}
                      </span>
                    </button>
                  )}
                </div>
